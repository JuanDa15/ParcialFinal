...
from Classes import Cañon
from Classes import BolaCañon
from Classes import Cerdo
from Classes import Pinchos
...
def StartGame(posx, posy):
    ...
    mapaa = pygame.image.load('Assets\Level1\Level1b.png')
    ...
    nom_archivo='Assets\Level1\Level1b.json'
    mapa_info = None
    ...
    for i in range(len(Dicc_Pinchos)):
        pincho = Pinchos.pincho([(Dicc_Pinchos[i]['x']),(Dicc_Pinchos[i]['y'])],Dicc_Pinchos[i]['width'],Dicc_Pinchos[i]['height'])
        Puas.add(pincho)
    ...
    for i in range(len(Dicc_Cañones)):
        C = Cañon.cañon([(Dicc_Cañones[i]['x']),(Dicc_Cañones[i]['y'])],(Dicc_Cañones[i]['width']),(Dicc_Cañones[i]['height']))
        if Dicc_Cañones[i]['name'] == 'False':
            Cañon.Direccion = False
        else:
            Cañon.Direccion = True
        Cañones.add(C)
    ...
    while (True):
        ...
        for c in Cañones:
            if c.Disparo == 0:
                if c.Direccion == True:
                    B = BolaCañon.bolaCañon([c.rect.x, c.rect.y + 4], 5)
                    B.Bloques = Bloques
                    BolasCañon.add(B)
                    c.Disparo = 80
                else:
        ...
        if j.rect.x > limitemovimiento:
            j.velx=0
        if j.rect.x < 0:
            H1.StartGame(limitemovimiento, j.rect.y)
