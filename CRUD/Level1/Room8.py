...
def StartGame(j,posx, posy):
    ...
    mapaa = pygame.image.load('Assets\Levels\Level1\Level1h.png')
    ...
    nom_archivo='Assets\Levels\Level1\Level1h.json'
    mapa_info = None
    with open(nom_archivo) as info:
        mapa_info=json.load(info)
    info.close()

    Dicc_Colisiones=mapa_info['layers'][11]['objects']
        ...
        for j in jugadores:
            ...
            for j in jugadores:
                if j.rect.y >= Constants.Height + 10:
                    Room7.StartGame(j, j.rect.x, - 10)

                if j.rect.bottom < -20:
                    Room9.StartGame(j, j.rect.x, Constants.Height - 30)
