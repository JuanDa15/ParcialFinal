...
def StartGame(j,posx, posy):
    ...
    mapaa = pygame.image.load('Assets\Levels\Level1\Level1e.png')
    ...
    nom_archivo='Assets\Levels\Level1\Level1e.json'
    mapa_info = None
    with open(nom_archivo) as info:
        mapa_info=json.load(info)
    info.close()

    Dicc_Colisiones=mapa_info['layers'][2]['objects']
    Dicc_Plataformas= mapa_info['layers'][16]['objects']
    #Dicc_Cañones= mapa_info['layers'][16]['objects']
    Dicc_Pinchos= mapa_info['layers'][5]['objects']
        ...
        for j in jugadores:
            ...
            for j in jugadores:
                if j.rect.y >= Constants.Height + 10:
                    Room4.StartGame(j, 741, 25)

        if j.rect.left > limitemovimiento:
            Room6.StartGame(j,0, j.rect.y)
