...
from CRUD.Level2 import Room10
from CRUD.Level2 import Room8
...
def StartGame(j ,posx, posy):
    ...
    mapaa = pygame.image.load('Assets\Levels\Level2\Level2i.png')
    ...
    nom_archivo='Assets\Levels\Level2\Level2i.json'
    mapa_info = None
    with open(nom_archivo) as info:
        mapa_info=json.load(info)
    info.close()

    Dicc_Colisiones=mapa_info['layers'][5]['objects']
    ...
    while (True):
        ...
        if j.rect.left > Constants.Width:
            Room10.StartGame(j,0, j.rect.y - 5)

        if j.rect.right < 5:
            Room8.StartGame(j,Constants.Width - 21, j.rect.y - 5)
